$(function() {
  ...
  addFragment({
    ...
    code: ["if(ppo.isMobile()) { document.querySelector('.title').style.color = '#fff'; }"],
  ...
  addFragment({
    ...
    code: ["if(ppo.isWeixin()) { document.querySelector('.title').style.color = '#fff'; }"],
  ...
  addFragment({
    ...
    code: ['var args = ppo.args(arguments); \n', 'var args = ppo.args(arguments, 3); \n']
  });
...
function addFragment(data) {
  ...
  var side = [
    '<li>- <a href="#' + data.name.replace(/\s/gi, '') + '">' + data.name + '</a></li>'
  ].join('');
