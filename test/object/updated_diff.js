  ...
  describe('base case', () => {
    describe('equal', () => {
      test.each([
        ...
        expect(bbo.updatedDiff(value, value)).toEqual({});
      });
    });
    ...
    describe('not equal and not object', () => {
      test.each([
        ...
        (lhs, rhs) => {
          expect(bbo.updatedDiff(lhs, rhs)).toEqual(rhs);
        }
      );
    });
  });
  ...
  describe('recursive case', () => {
    describe('object', () => {
      ...
      test('returns empty object when deleted from right hand side', () => {
        expect(bbo.updatedDiff({ a: 1, b: { c: 2 } }, { a: 1 })).toEqual({});
      });

      test('returns empty object when a key value has been added', () => {
        expect(bbo.updatedDiff({ a: 1 }, { a: 1, b: 2 })).toEqual({});
      });

      test('returns subset of right hand side with updated date', () => {
        expect(bbo.updatedDiff({ date: new Date('2016') }, { date: new Date('2017') })).toEqual({
          date: new Date('2017')
        });
      });
    });
    ...
    describe('date', () => {
      test('returns empty object when dates are equal', () => {
        expect(bbo.updatedDiff(new Date('2016'), new Date('2016'))).toEqual({});
      });
    });
    ...
    describe('object create null', () => {
      test('returns right hand side value when given objects are different at root', () => {
        ...
        rhs.a = 2;
        expect(bbo.updatedDiff(lhs, rhs)).toEqual({ a: 2 });
      });
      ...
      test('returns subset of right hand side value when sibling objects differ', () => {
        ...
        rhs.a = { b: 1 };
        rhs.c = 3;
        expect(bbo.updatedDiff(lhs, rhs)).toEqual({ c: 3 });
      });
      ...
      test('returns subset of right hand side value when nested values differ', () => {
        ...
        const rhs = Object.create(null);
        rhs.a = { b: 1, c: 3 };
        expect(bbo.updatedDiff(lhs, rhs)).toEqual({ a: { c: 3 } });
      });
      ...
      test('returns subset of right hand side with updated date', () => {
        ...
        expect(bbo.updatedDiff(lhs, rhs)).toEqual({ date: new Date('2017') });
      });
    });
  });
});
