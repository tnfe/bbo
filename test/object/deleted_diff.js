  ...
  describe('base case', () => {
    describe('equal', () => {
      test.each([
        ...
        expect(bbo.deletedDiff(value, value)).toEqual({});
      });
    });
    ...
    describe('not equal and not object', () => {
      test.each([
        ...
        expect(bbo.deletedDiff(lhs, rhs)).toEqual({});
      });
    });
  });
  ...
  describe('recursive case', () => {
    ...
    describe('object create null', () => {
      test('returns keys as undefined when deleted from right hand side root', () => {
        ...
        lhs.b = 2;
        rhs.a = 1;
        expect(bbo.deletedDiff(lhs, rhs)).toEqual({ b: undefined });
      });
      ...
      test('returns keys as undefined when deeply deleted from right hand side', () => {
        ...
        rhs.a = { b: 1 };
        rhs.c = {};
        expect(bbo.deletedDiff(lhs, rhs)).toEqual({ c: { d: undefined } });
      });
      ...
      test('returns subset of right hand side with deleted date', () => {
        ...
        expect(bbo.deletedDiff({ date: new Date('2016') }, rhs)).toEqual({ date: undefined });
      });
    });
  });
});
