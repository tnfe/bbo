...
void function(exports) {
  ...
  /**
     * 默认绑定的事件
     */
  let defaultEvents = ["mousedown", "click", "dblclick"];
  ...
  exports.on = function(element, callback, events) {
    ...
    handler++;
    let eventItem = eventDict[handler] = {
      target: element,
      events: {},
      callback: callback
    };
    lib.each(events, function(eventName) {
      ...
      eventItem.events[eventName] = function(e) {
        ...
        scanParent(target, function(node) {
          ...
          let command = node.getAttribute("event-" + eventName);
          if (!command) { // 处理简写
            switch (eventName) {
              case "click":
  ...
  exports.un = function(handler) {
    let eventItem = eventDict[handler];
    if (!eventItem) return;
    for (let eventName in eventItem.events) {
